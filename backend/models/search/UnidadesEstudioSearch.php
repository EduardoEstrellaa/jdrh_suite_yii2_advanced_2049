<?php

namespace backend\models\search;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use backend\models\UnidadesEstudio;

/**
 * UnidadesEstudioSearch represents the model behind the search form of `backend\models\UnidadesEstudio`.
 */
class UnidadesEstudioSearch extends UnidadesEstudio
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'semestres_id', 'horas_semana', 'horas_semestre'], 'integer'],
            [['nombre', 'descripcion_general'], 'safe'],
            [['creditos'], 'number'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = UnidadesEstudio::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'semestres_id' => $this->semestres_id,
            'creditos' => $this->creditos,
            'horas_semana' => $this->horas_semana,
            'horas_semestre' => $this->horas_semestre,
        ]);

        $query->andFilterWhere(['like', 'nombre', $this->nombre])
            ->andFilterWhere(['like', 'descripcion_general', $this->descripcion_general]);

        return $dataProvider;
    }
}
